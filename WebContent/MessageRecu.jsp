...
<div id="bg"  style="padding-top: 10vh;margin-top:auto;padding-bottom:20px;" >
    <div class="row inscription-text animated bounceInLeft centre">
        ...
        <h2 style="text-transform: uppercase;color:#1f8d94;text-align: center;font-weight: bold;padding-top:10%">Message Reçu</h2>
        <p style="color:#e9f7f8;text-transform: centre;">
            Ci-dissous vous trouvez les infos des personnes qui sont interessé a vos annonces. 
          </p>
        </div>
          </div>
    <div class="container inscription-text animated bounceInLeft" style="margin-bottom:10%;margin-top:2%">
	         ...
	         <c:if test="${list!=null }">
				 <c:forEach items="${list}" var="ct">
					<tr style="background-color:white;color:#053a3d;font-weight:bold">
		                 <td ><c:out value="${ct.getNom_emeteur() }"></c:out></td>
		                <td ><c:out value="${ct.getPrenom_emeteur() }"></c:out></td>
		                <td ><c:out value="${ct.getEmail_emeteur() }"></c:out></td>
		                <td><c:out value="${ct.getMessage() }"></c:out></td>
		              </tr>
              	</c:forEach>
              </c:if>
            </tbody>
